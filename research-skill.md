...
function RESEARCH(question): subquestions = DECOMPOSE(question) for each q in subquestions: if q is atomic: answer[q] = INVESTIGATE(q) else: answer[q] = RESEARCH(q) return SYNTHESISE(answer)
...
loop until evidence_sufficient or limits_reached: gather sources classify sources (primary, secondary, tertiary) evaluate credibility, incentives, and bias extract claims cross‑verify claims across independent sources identify contradictions update evidence map
...
loop until no contradictions remain: scan for internal inconsistencies scan for unsupported leaps scan for ambiguous terms if contradiction found: trace back to source revise or flag uncertainty
...
for lens in [technical, economic, regulatory, historical, behavioural]: re-evaluate findings through lens add new insights or constraints
...
function REVIEW(section): if section is atomic: validate clarity, evidence, and logic else: for each subsection: REVIEW(subsection)


Stop only when:
- Every section is justified  
- We’ve travelled all the threads synthesised all the information 
- The structure is clear and integrated 
- Every claim is sourced or if it’s an inferred claim is labelled as inference  
